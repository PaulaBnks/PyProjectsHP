...
##############################################
## Generate Summaries from Embeddings       ##
...
def get_embeddings_for_account(account_id, days=730):  # Default: last 2 years
    """Fetch all embeddings (Notes, Tasks, Events) + raw text + created_date + object_type"""
    cutoff_date = datetime.now() - timedelta(days=days)
    cutoff_str = cutoff_date.strftime("%Y-%m-%d")

    query = f"""
        SELECT id, raw_text, embedding, created_date, object_type
    ...
    print(f"🧠 Loaded {rows.total_rows} records for account {account_id}")
    return [(row.id, strip_html(row.raw_text), row.embedding, row.created_date, row.object_type) for row in rows]
...
def find_most_relevant_notes(query_embedding, embeddings_data, decay_factor=0.95, top_n=5):
    """Find top N most similar notes based on hybrid score (semantic + recency + object type boost)"""
    similarities = []
    now = datetime.now()
    ...
    for item_id, raw_text, embedding, created_date, object_type in embeddings_data:
        ...
        try:
            ...
            weighted_score = sim * time_weight * boost_weights.get(object_type, 1.0)
        except Exception:
            weighted_score = sim

        similarities.append((weighted_score, raw_text[:1000], object_type))  # Limit context size

    similarities.sort(reverse=True, key=lambda x: x[0])
    return similarities[:top_n]  # Top N matches


def ask_question_with_context(question, relevant_notes):
    """Ask Gemini a question using only the most relevant snippets (Notes, Tasks, Events)"""

    context_block = "\n".join([f"[{obj}] {text}" for _, text, obj in relevant_notes])

    full_prompt = f"""
{COSUNO_CONTEXT}

Du bist ein hilfreicher Assistent für Salesforce-Vertriebsmitarbeiter bei Cosuno. Deine Aufgabe ist es, basierend auf den bereitgestellten Notizen, Tasks und Events prägnante und gut strukturierte Antworten zu liefern.
...
- Falls keine Info gefunden wird, antworte mit: "(Keine relevante Info gefunden)"
...
if not embeddings_data:
    ...
    print(f"✅ Found {len(embeddings_data)} embedded items.\n")
    ...
    for question, section in questions:
        ...
        relevant_notes = find_most_relevant_notes(query_emb, embeddings_data)
        ...
        # print("\n📎 Verwendete Notizen:")
        # for idx, (sim, text, obj) in enumerate(relevant_notes):
        #     print(f"\nNote {idx+1} ({obj}, Ähnlichkeit: {sim:.4f})\n{text[:200]}...")