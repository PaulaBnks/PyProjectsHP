...
from gemini_prompter import (
    ...
    create_prompt_question_19, create_prompt_question_20, create_prompt_question_21, create_prompt_question_22, create_prompt_question_23,create_prompt_question_24)
...
#DOCUMENT_ID = '1hISTyvQ_r-DVI3n3kfRQ9WGQiHcBvLMgh48M4zIuhkc' #English version
DOCUMENT_ID = '1n7CRHV2XgzhIbOWWNHQ_PVVM9bW6Ruaoq-RNHgpOuIU' #German version
...
def main():
    try:
        ...
        query = """
            SELECT Id, Name,Website,Customer_Since_Start_Date_First_Contra__c, Acquired_Licenses__c, Active_Users__c, Number_of_User_with_75_Activity_Score__c, Acquired_Tendering_volume__c, Bid_Packages_Tot_last_365_Days__c, NumberOfEmployees 
            FROM Account 
        ...
        for acc in accounts:
            ...
            signing_date = acc.get("Customer_Since_Start_Date_First_Contra__c", "Not specified")
            start_date_dt = datetime.strptime(signing_date, "%Y-%m-%d")
            end_of_window = start_date_dt + timedelta(days=60)
            ...
            filtered_success_plan_notes = []
            for note in all_notes:
                if note['parsed_date'] >= start_date_dt and note['parsed_date'] <= end_of_window:
                    filtered_success_plan_notes.append(note['text'])

            # Join the filtered success plan notes
            success_plan_notes = "\n\n".join(filtered_success_plan_notes)
        ...
        #     \n\n"""
            ...
            #Build Prompt for Question 24
            prompt24 = create_prompt_question_24(
                account_name=account_name,                
                meeting_notes = success_plan_notes,
                start_date=start_date_dt,
                end_date=end_of_window                  
            )
            # Ask Gemini Question 24
            try:
                print("\n📌 Sending to Gemini (Question 24):")
                response24 = model.generate_content(prompt24)
                answer24 = response24.candidates[0].content.parts[0].text.strip()
            except Exception as e:
                print(f"❌ Gemini error 24: {e}")
                candidate = "(Error generating summary)"

            # Combine Results
            full_summary += f"""
            ## 24. Customer Success Plan:           
            {answer24}
            \n\n"""
